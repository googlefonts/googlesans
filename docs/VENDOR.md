...
## Writing Custom mark or kern Feature Code

If you need to write manual feature code for mark adjustment (e.g. `mark`, `mkmk`, `blwm` features) or kerning (`kern` or `dist` features) and want it to mix with autogenerated code, use `# Automatic Code` comments inside your manually written block (like Glyphs.app does):

```text
feature dist {
	# Automatic Code

    # The following code is inserted AFTER autogenerated kerning (dist) code:

    @BNG_eMatra_short2 = [eMatra-beng.short2 aiMatra-beng.short2 eMatra-beng.init.short2 aiMatra-beng.init.short2];
    pos @BNG_eMatra_short2' 20 ph-beng.half1 ba-beng.blwf2;
    pos @BNG_eMatra_short2' 20 s-beng.half4 ba-beng.blwf3;
    pos @BNG_eMatra_short2' 10 m-beng.half3 pha-beng.half1;
} dist;
```
...
```text
feature dist {
    # The following code is inserted BEFORE autogenerated kerning (dist) code:
