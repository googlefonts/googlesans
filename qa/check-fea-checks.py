...
import uharfbuzz
from fontbakery.prelude import check, condition, PASS, FAIL, SKIP
...
def is_italic(self: Font):
    return "Italic" in self.ttFont.reader.file.name


@condition(Font)
def is_not_italic(self: Font):
    return "Italic" not in self.ttFont.reader.file.name


@condition(Font)
def is_not_variable_font(self: Font):
    return "fvar" not in self.ttFont.keys()


@condition(Font)
def is_variable_font(self: Font):
    return "fvar" in self.ttFont.keys()


@condition(Font)
def hb_font(self: Font):
    with open(self.file, "rb") as fontfile:
