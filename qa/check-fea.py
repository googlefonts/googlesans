        ...
        if shaped_texts == shaped_texts_expected:
            ...
            if "fvar" in tt:
                ...
                for key, shaped_text in shaped_texts.items():
                    try:
                        shaped_texts_expected_instance = shaped_texts_expected[key]
                    except KeyError as e:
                        yield FAIL, (
                            f"{shaping_file}: No entry found for instance {str(e)} of "
                            f"{filename.name}"
                        )
                        continue
                    if shaped_text != shaped_texts_expected_instance:
                        shaped_texts_str = textwrap.indent(
                            "\n".join(shaped_text), "\t  "
                        )
                        shaped_texts_expected_str = textwrap.indent(
                            "\n".join(shaped_texts_expected_instance), "\t  "
                        )
                        yield FAIL, (
